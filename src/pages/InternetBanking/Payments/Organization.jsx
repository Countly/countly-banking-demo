...
const Organization = () => {
  ...
  const pay = () => {
    ...
    Countly.add_event({
      key: 'Payment',
      segmentation: { type: 'Organization' },
    });
    toast('"Payment" event sent with "Organization" segmentation', {
      position: 'top-right',
      autoClose: 5000,
        ...
        <form>
          <div className="relative">
            ...
            <select className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 mb-6 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
              <option>Organization X</option>
              <option>Organization Y</option>
              <option>Organization Z</option>
            </select>
